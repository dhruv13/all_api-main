...
class dio19 extends StatefulWidget {
  const dio19({Key? key}) : super(key: key);

  @override
  State<dio19> createState() => _dio19State();
}

class _dio19State extends State<dio19> {
  ...
  pro_Data() async {
    try {
      var response = await Dio().get('https://dummyjson.com/products/search?q=Laptop');
      print(response);
      l = response.data;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Dummy Laptop  dio"),
      ),
      body: status
          ? ListView.builder(
              shrinkWrap: true,
